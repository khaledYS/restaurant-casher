    ...
    <>
      <div className=" h-full w-full overflow-auto ">
        <div className="relative pt-8 px-2 pb-8 overflow-auto  h-full w-full ">
          <span
            style={{ transition: "all .2s ease-in-out", zIndex: "8" }}
            title="close this bill"
            className="cursor-pointer absolute top-4 right-6 text-3xl text-gray-300 hover:text-white drop-shadow-lg py-1 px-6 bg-gray-700 hover:bg-gray-500 rounded-lg"
            onClick={() => {
              setCurrentBill(null);
            }}
          >
          ...
          <div className="my-2 text-xl ">
            <div className="title text-3xl text-center mb-4 flex flex-wrap justify-center items-center">
              ...
              {employee.name == currentBill.submittedBy &&
                !currentBill.deleted && (
                  <Link
                    className="flex ml-4 w-fit px-2 py-1 rounded-md border-2 border-zinc-300 hover:bg-zinc-300"
                    to={`/welcome/${currentBranchId}/order/${currentBill.id}`}
            ...
            <div className="text-center">
              ...
              <div>
                <div
                  className={`${
                    currentBill.finished ? "finished" : "notfinished"
                  }`}
                >
                  {`${currentBill.finished ? "Finished" : "Not Finished"}`}
                  {currentBill.finished && (
                    <div>BY: {currentBill.finishedBy}</div>
                  )}
                </div>
              </div>
            </div>
          ...
          {currentBill.lastEdit?.length ? (
            <details className="mx-auto w-[90%] break-words bg-gray-600 relative mt-6 p-2 pr-10 rounded-lg text-white shadow-[0_.1rem_1rem_-.4rem_rgb(0,0,0)]">
              <summary className="cursor-pointer w-full ">
                Last edit is on{" "}
                <PrettyDate date={currentBill.lastEdit[0].toDate()} />{" "}
              ...
              {currentBill.lastEdit.map((el, ind) => {
                return (
                  <p key={uuidv4()} className="ml-6">
                    <span className="font-bold">{ind + 1}-</span>{" "}
                    {ind != 0 && "also"} edited on{" "}
                    <PrettyDate date={el.toDate()} />{" "}
                  </p>
                );
              })}
            </details>
          ...
          {/* orders table */}
          <table
            className="w-full border-2 border-gray-700 mt-5 shadow-xl shadow-gray-300"
          ...
          >
            <thead>
              <tr>
                ...
                <th className="border-black border-2  p-1" title="Quantity">
                  Q
                </th>
                <th
            ...
            <tbody>
              {currentBill.bill &&
                getTheQuantity(currentBill.bill).map((item, index) => {
                  return (
                    <tr key={uuidv4()} title={item.billItemId}>
                      <td className="border-black border-2 p-1 text-center">
                        {index + 1}
                      </td>
                      <td className="border-gray-300 border-2 p-1">
                        {item.category}
                      </td>
                      <td className="border-gray-300 border-2 p-1">
                        {item.title}
                      </td>
                      <td className="border-gray-300 border-2 p-1 text-center">
                        {item.cost}$
                      </td>
                      <td className="border-gray-300 border-2 p-1 text-center">
                        {item.quantity}
                      ...
                      <td
                        className="border-gray-300 border-2 p-1 text-center"
                        title="Quantity amount"
                      >
                        {item.quantityAmount ? item.quantityAmount : item.cost}$
                      </td>
                    </tr>
                  );
                })}
          ...
          <div className="flex w-full justify-around  bg-gray-300 py-2 pl-2 border-t-2 border-black shadow-xl shadow-gray-400">
            ...
            <span>
              {" "}
              Final price :{" "}
              <span className="bg-yellow-300 rounded-md py-1 px-2">
                {currentBill.billTotalBalance +
                  (currentBill.billTotalBalance * employee.tax) / 100}
                $
              </span>
            </span>
          </div>
              ...
              currentBill.finished && !currentBill.deleted ? (
                <button
                  ...
                  onClick={async () => {
                    // confirmation
                    if (
                      !window.confirm("Do you really want to unfinish the bill")
                    )
                      return;

                    const data = await setDoc(
                      doc(db, "bills/" + currentBill.id.toString()),
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const confirmation = window.confirm(
                      "Do you really want to finish The Bill"
                    );
                    if (!confirmation) return;

                    await setDoc(
                      doc(db, "bills/" + currentBill.id.toString()),
                      { finished: true, finishedBy: employee.name },
                      { merge: true }
                    );
                  }}
                >
                ...
                currentBill.deleted && (
                  <div className="text-center text-lg text-red-600">
                    * Recover this bill to{" "}
                    {currentBill.finished ? "Unfinish" : "finish"}{" "}
                    {employee.name == currentBill.submittedBy && "or edit"}.{" "}
                  </div>
                )
              )
            }
              ...
              currentBill.deleted == false ? (
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (
                      window.confirm(
                        "Do you really want to delete The Bill ?! | confirmation 1"
                      )
                    ) {
                      if (
                        window.confirm(
                        ...
                        await setDoc(
                          doc(db, "bills/" + currentBill.id.toString()),
                          { deleted: true, deletedBy: employee?.name },
                          { merge: true }
                        );
                      }
                    }
                ...
                currentBill.deletedBy == employee.name && (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      if (
                        window.confirm(
                          "Do you really want to recover The Bill ?! | confirmation 1"
                        )
                      ) {
                        if (
                          window.confirm(
                            "Do you really want to recover The Bill ?! | confirmation 2"
                          )
                        ) {
                          await setDoc(
                            doc(db, "bills/" + currentBill.id.toString()),
